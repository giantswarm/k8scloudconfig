...
- path: /opt/k8s-addons
  ...
  content: |
      ...
      while [ "$(/usr/bin/docker run $IMAGE get cs | grep Healthy | wc -l)" -ne "3" ]; \
      ...
      do
          while
              /usr/bin/docker run $IMAGE --kubeconfig=/etc/kubernetes/config/kubelet-kubeconfig.yml apply -f /srv/$manifest
              [ "$?" -ne "0" ]
          do
      ...
      while
          ...
          /usr/bin/docker run $IMAGE --kubeconfig=/etc/kubernetes/config/kubelet-kubeconfig.yml -n kube-system  get ds calico-node 2>/dev/null >/dev/null
          RET_CODE_1=$?
          eval $(/usr/bin/docker run $IMAGE --kubeconfig=/etc/kubernetes/config/kubelet-kubeconfig.yml -n kube-system get ds calico-node | tail -1 | awk '{print "[ \"" $2"\" -eq \""$4"\" ] "}')
          RET_CODE_2=$?
