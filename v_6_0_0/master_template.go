...
systemd:
  ...
  - name: k8s-setup-download-hyperkube.service
    ...
    contents: |
      ...
      ExecStart=/bin/bash -c "/usr/bin/docker cp $NAME:/usr/local/bin/kube-apiserver /opt/bin/kube-apiserver"
      ExecStart=/bin/bash -c "/usr/bin/docker cp $NAME:/usr/local/bin/kube-controller-manager /opt/bin/kube-controller-manager"
      ExecStart=/bin/bash -c "/usr/bin/docker cp $NAME:/usr/local/bin/kube-proxy /opt/bin/kube-proxy"
      ExecStart=/bin/bash -c "/usr/bin/docker cp $NAME:/usr/local/bin/kube-scheduler /opt/bin/kube-scheduler"
