...
systemd:
  ...
  - name: k8s-setup-download-hyperkube.service
    ...
    contents: |
      ...
      ExecStartPre=/bin/bash -c "/usr/bin/docker cp $NAME:/usr/local/bin/kube-apiserver /opt/bin/kube-apiserver"
      ExecStartPre=/bin/bash -c "/usr/bin/docker cp $NAME:/usr/local/bin/kube-controller-manager /opt/bin/kube-controller-manager"
      ExecStartPre=/bin/bash -c "/usr/bin/docker cp $NAME:/usr/local/bin/kube-proxy /opt/bin/kube-proxy"
      ExecStartPre=/bin/bash -c "/usr/bin/docker cp $NAME:/usr/local/bin/kube-scheduler /opt/bin/kube-scheduler"
      ...
      ExecStartPre=/bin/bash -c "echo 'echo hyperkube kube-apiserver "${@}"' > /opt/bin/kube-apiserver && chmod +x /opt/bin/kube-apiserver"
      ExecStartPre=/bin/bash -c "echo 'echo hyperkube kube-controller-manager "${@}"' > /opt/bin/kube-controller-manager && chmod +x /opt/bin/kube-controller-manager"
      ExecStartPre=/bin/bash -c "echo 'echo hyperkube kube-proxy "${@}"' > /opt/bin/kube-proxy && chmod +x /opt/bin/kube-proxy"
      ExecStartPre=/bin/bash -c "echo 'echo hyperkube kube-scheduler "${@}"' > /opt/bin/kubelet && chmod +x /opt/bin/kubelet"
